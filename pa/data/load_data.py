...
from datetime import date
from pa.models import STATE_TYPE_CONFIRM, STATE_TYPE_DRAFT, LkpItem,TblCompany, TblCompanyCommitmentDetail, TblCompanyProduction, TblCompanyProductionLicense,TblCompanyCommitmentMaster, TblCompanyRequestDetail, TblCompanyRequestMaster
...
def import_commitment_mokhalafat(file_name='commitment_mokhalafat.csv',items=['متبقي رسوم عقد المخلفات', 'متبقي بونص توقيع عقد مخلفات', 'متبقي سداد الهيئة رسوم عقد المخلفات', 'متبقي سداد الهيئة بونص توقيع عقد مخلفات']):
...
def import_commitment_exp(file_name='commitment_exp.csv',items=['أخري/تسوية','بونص توقيع إتفاقية الإمتياز','ممثل حكومة','دعم فني','إيجار ارض لإتفاقيات الإمتياز']):
    with open('./pa/data/'+file_name, newline='') as csvfile:
        reader = csv.reader(csvfile, delimiter=',')
        next(reader, None)  # skip the headers
        for row in reader:
...
def run_all():
    ...
    print("Importing commitment_mokhalafat...")
    import_commitment_mokhalafat('commitment_mokhalafat.csv')
    import_commitment_mokhalafat('commitment_mokhalafat2.csv')
    import_commitment_mokhalafat('commitment_mokhalafat3.csv')
    import_commitment_mokhalafat('commitment_mokhalafat4.csv', items=['رسوم عقد حجر معدنين / رسوم العقد الثلاثي', 'متبقي بونص توقيع حجر معدنيين', 'متبقي سداد الهيئة رسوم حجر معدنيين', 'متبقي سداد الهيئة  بونص توقيع حجر معدنيين'])
    
    print("Importing commitment_emtiaz...")
    import_commitment_emtiaz()
    
    print("Importing commitment_sageer...")
    import_commitment_sageer()
    
    print("Importing commitment_exp...")
    import_commitment_exp()
    
    print("Importing mokhalafat_request (SDG)...")
    import_mokhalafat_request(currency="sdg")
    
    print("Importing mokhalafat_request (Dollar)...")
    import_mokhalafat_request(currency="dollar")
    
    print("Cleaning up zero total requests...")
    remove_request_with_zero_total()
