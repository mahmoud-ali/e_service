...
from ..forms import AppChangeWorkProcedureForm

from ..workflow import STATE_CHOICES,SUBMITTED,ACCEPTED,APPROVED,REJECTED,send_transition_email,get_sumitted_responsible
from ..tables import AppChangeWorkProcedureTable

from .application import ApplicationListView, ApplicationCreateView, ApplicationReadonlyView

class AppChangeWorkProcedureListView(ApplicationListView):
    model = AppChangeWorkProcedure
    table_class = AppChangeWorkProcedureTable
    menu_name = "profile:app_change_work_procedure_list"
    title = _("List of change work procedure")
...
class AppChangeWorkProcedureCreateView(ApplicationCreateView):
    model = AppChangeWorkProcedure
    form_class = AppChangeWorkProcedureForm
    menu_name = "profile:app_change_work_procedure_list"
    title = _("Add change work procedure")
    ...
    def form_valid(self, form):
        ...
        self.object.study_about_change_reason_file = self.request.FILES["study_about_change_reason_file"]
        self.object.study_about_new_suggestion_file = self.request.FILES["study_about_new_suggestion_file"]
        self.object.save()
...
class AppChangeWorkProcedureReadonlyView(ApplicationReadonlyView):
    model = AppChangeWorkProcedure
    form_class = AppChangeWorkProcedureForm
    menu_name = "profile:app_change_work_procedure_list"
    title = _("Show change work procedure")
