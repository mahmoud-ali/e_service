...
from ..forms import AppLocalPurchaseForm

from ..workflow import STATE_CHOICES,SUBMITTED,ACCEPTED,APPROVED,REJECTED,send_transition_email,get_sumitted_responsible
from ..tables import AppLocalPurchaseTable

from .application import ApplicationListView, ApplicationCreateView, ApplicationReadonlyView

class AppLocalPurchaseListView(ApplicationListView):
    model = AppLocalPurchase
    table_class = AppLocalPurchaseTable
    menu_name = "profile:app_local_purchase_list"
    title = _("List of Local Purchases")
...
class AppLocalPurchaseCreateView(ApplicationCreateView):
    model = AppLocalPurchase
    form_class = AppLocalPurchaseForm
    menu_name = "profile:app_local_purchase_list"
    title = _("Add Local Purchase")
...
class AppLocalPurchaseReadonlyView(ApplicationReadonlyView):
    model = AppLocalPurchase
    form_class = AppLocalPurchaseForm
    menu_name = "profile:app_local_purchase_list"
    title = _("Show Local Purchase")
