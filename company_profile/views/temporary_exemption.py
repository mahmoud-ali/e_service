...
from ..forms import AppTemporaryExemptionForm

from ..workflow import STATE_CHOICES,SUBMITTED,ACCEPTED,APPROVED,REJECTED,send_transition_email,get_sumitted_responsible
from ..tables import AppTemporaryExemptionTable

from .application import ApplicationListView, ApplicationCreateView, ApplicationReadonlyView

class AppTemporaryExemptionListView(ApplicationListView):
    model = AppTemporaryExemption
    table_class = AppTemporaryExemptionTable
    menu_name = "profile:app_temporary_exemption_list"
    title = _("List of Temporary Exemptions")
...
class AppTemporaryExemptionCreateView(ApplicationCreateView):
    model = AppTemporaryExemption
    form_class = AppTemporaryExemptionForm
    menu_name = "profile:app_temporary_exemption_list"
    title = _("Add Temporary Exemptions")
...
class AppTemporaryExemptionReadonlyView(ApplicationReadonlyView):
    model = AppTemporaryExemption
    form_class = AppTemporaryExemptionForm
    menu_name = "profile:app_temporary_exemption_list"
    title = _("Show Temporary Exemptions")
