...
from ..forms import AppTechnicalFinancialReportForm

from ..workflow import STATE_CHOICES,SUBMITTED,ACCEPTED,APPROVED,REJECTED,send_transition_email,get_sumitted_responsible
from ..tables import AppTechnicalFinancialReportTable

from .application import ApplicationListView, ApplicationCreateView, ApplicationReadonlyView

class AppTechnicalFinancialReportListView(ApplicationListView):
    model = AppTechnicalFinancialReport
    table_class = AppTechnicalFinancialReportTable
    menu_name = "profile:app_technical_financial_report_list"
    title = _("List of technical & financial reports")
...
class AppTechnicalFinancialReportCreateView(ApplicationCreateView):
    model = AppTechnicalFinancialReport
    form_class = AppTechnicalFinancialReportForm
    menu_name = "profile:app_technical_financial_report_list"
    title = _("Add new technical & financial report")
    ...
    def form_valid(self, form):
        ...
        self.object.report_file = self.request.FILES["report_file"]
        self.object.other_attachments_file = self.request.FILES["other_attachments_file"]
        self.object.save()
...
class AppTechnicalFinancialReportReadonlyView(ApplicationReadonlyView):
    model = AppTechnicalFinancialReport
    form_class = AppTechnicalFinancialReportForm
    menu_name = "profile:app_technical_financial_report_list"
    title = _("Show technical & financial report")
