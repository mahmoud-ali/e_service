...
from ..models import AppWhomConcern
from ..forms import AppWhomConcernForm

from ..workflow import STATE_CHOICES,SUBMITTED,ACCEPTED,APPROVED,REJECTED,send_transition_email,get_sumitted_responsible
from ..tables import AppWhomConcernTable

from .application import ApplicationListView, ApplicationCreateView, ApplicationReadonlyView

class AppWhomConcernListView(ApplicationListView):
    model = AppWhomConcern
    table_class = AppWhomConcernTable
    menu_name = "profile:app_whom_concern_list"
    title = _("List of Whom may concern requests")
...
class AppWhomConcernCreateView(ApplicationCreateView):
    model = AppWhomConcern
    form_class = AppWhomConcernForm
    menu_name = "profile:app_whom_concern_list"
    title = _("Add new Whom may concern request")
    ...
    def form_valid(self, form):
        ...
        self.object.whom_attachement_file = self.request.FILES.get("whom_attachement_file",'')
        self.object.save()
...
class AppWhomConcernReadonlyView(ApplicationReadonlyView):
    model = AppWhomConcern
    form_class = AppWhomConcernForm
    menu_name = "profile:app_whom_concern_list"
    title = _("Show Whom may concern request")
