from django.test import TestCase
from django.urls import reverse, resolve
from django.conf import settings

from django.contrib.auth import get_user_model

from ..views import AppBorrowMaterialListView

class AppBorrowMaterialListTests(TestCase):
    view_class = AppBorrowMaterialListView
    template_name = 'company_profile/application_list.html'
    url_name = 'profile:app_borrow_list'
    url_path = '/app_borrow_materials/'
    html_contain = 'List of borrow materials'
    fixtures = ['test_data.yaml']
    def setUp(self):
        user = get_user_model()
#        user.objects.create_user(username="abc",email="abc@smrc.sd",password="123")
        
        self.client.cookies.load({settings.LANGUAGE_COOKIE_NAME: "en"})

        self.client.login(email="admin@smrc.sd",password="123")

        url = reverse(self.url_name)
        self.response = self.client.get(url)

    def test_status_code(self):
        self.assertEqual(self.response.status_code, 200)

    def test_template(self):
        self.assertTemplateUsed(self.response, self.template_name)

    def test_contains_correct_html(self):
        self.assertContains(self.response, self.html_contain)

    def test_url_resolves_view(self): 
        view = resolve(self.url_path)
        self.assertEqual(
            view.func.__name__,
            self.view_class.as_view().__name__
        )

    def test_not_accessable_after_logout(self):
        self.client.logout()
        url = reverse(self.url_name)
        self.response = self.client.get(url)
        self.assertNotEqual(self.response.status_code, 200)

