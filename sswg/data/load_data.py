...
def import_gold_travel(row,main_obj):
    try:
...
def import_mm_smrc(row,main_obj):
    try:
    ...
    with open('./sswg/data/'+file_name, newline='') as csvfile:
        ...
        for row in reader:
            ...
            try:
                ...
                if obj.state < 3 and import_gold_travel(row,obj):
                    obj.state = 3
                    obj.save()
                else:
                    if obj.state == 1:
                ...
                if obj.state < 4 and import_ssmo(row,obj):
                    obj.state = 4
                    obj.save()
                else:
                    continue

                if obj.state < 6 and import_mm_smrc(row,obj):
                    obj.state = 6
                    obj.save()
                else:
                    continue

                if obj.state < 8 and import_mocs(row,obj):
                    obj.state = 8
                    obj.save()
                else:
                    continue

                if obj.state < 9 and import_cbs(row,obj):
                    obj.state = 9
                    obj.save()
                else:
                    continue

                if obj.state < 11 and import_cf(row,obj):
                    obj.state = 11
                    obj.save()
                else:
                    continue
